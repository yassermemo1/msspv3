...
// Dynamic Dashboard Module Tables (Phase 1: Backend Integration)

// Table to manage dashboard containers
...
// Table to store the configuration for each widget
export const widgets = pgTable("widgets", {
  id: serial("id").primaryKey(),
  dashboardId: integer("dashboard_id").notNull().references(() => dashboards.id, { onDelete: 'cascade' }),
  title: text("title").notNull(),
  widgetType: text("widget_type").notNull(), // 'kpi', 'bar_chart', 'line_chart', 'table'
  dataSource: text("data_source").notNull(), // Name of a table/view in the DB
  config: jsonb("config").notNull(), // The "recipe" for the query
  position: jsonb("position").notNull(), // The grid position: {x, y, w, h}
  createdAt: timestamp("created_at").notNull().defaultNow(),
  updatedAt: timestamp("updated_at").notNull().defaultNow(),
});

// Dynamic Dashboard Module Types
...
// Dynamic Dashboard Module Relations
...
// Dynamic Dashboard Module Schemas
...
export const externalSystems = pgTable("external_systems", {
  id: serial("id").primaryKey(),
  systemName: text("system_name").notNull().unique(), // 'jira', 'grafana', etc.
  displayName: text("display_name").notNull(), // 'Jira', 'Grafana', etc.
  baseUrl: text("base_url").notNull(),
  authType: text("auth_type").notNull(), // 'basic', 'bearer', 'api_key'
  authConfig: jsonb("auth_config"), // Store auth configuration (not credentials)
  apiEndpoints: jsonb("api_endpoints"), // Store endpoint templates
  isActive: boolean("is_active").notNull().default(true),
  createdBy: integer("created_by").notNull().references(() => users.id),
  createdAt: timestamp("created_at").notNull().defaultNow(),
  updatedAt: timestamp("updated_at").notNull().defaultNow(),
