...
import { 
  ...
  users, clients, contracts, proposals, services, serviceScopes, financialTransactions,
  serviceAuthorizationForms, certificatesOfCompliance, hardwareAssets,
  auditLogs, changeHistory, securityEvents, dataAccessLogs,
  dashboardWidgets, userDashboards, dashboardWidgetAssignments,
  dashboards, widgets, externalSystems, clientExternalMappings,
...
import { 
  customFields, customFieldValues, documents,
  documentVersions, documentAccess, userSettings,
  clientContacts, licensePools, clientLicenses, individualLicenses,
  serviceScopes, clientHardwareAssignments,
  clientTeamAssignments,
  clientSatisfactionSurveys, clientFeedback,
  companySettings, dataSources, dataSourceMappings, integratedData,
  dashboardWidgetAssignments,
  dashboards, widgets, externalSystems,
  clientExternalMappings,
  pagePermissions, savedSearches, searchHistory, services
} from "@shared/schema";
...
import { cors } from "@hono/node-server/cors";
...
import { type Client, type InsertClient, type User, 
  ...
  type InsertClientExternalMapping, type UserDashboardSettings } from "@shared/schema";
import { apiLimiter } from "./middleware/rate-limiter";
import { logRequestInfo } from "./middleware/request-logger";
...
// Create service scope for a contract
  ...
  app.get("/api/dashboards", requireAuth, async (req, res) => {
    try {
      const userDashboards = await db.select()
        .from(dashboards)
        .where(eq(dashboards.userId, req.user.id));
  ...
  app.get("/api/dashboards/:id", requireAuth, async (req, res) => {
    try {
      ...
      const dashboardWidgets = await db.select()
        .from(widgets)
  ...
  app.delete("/api/dashboards/:id", requireAuth, async (req, res) => {
    try {
      ...
      // Delete all widgets first (cascade delete)
  ...
  app.post("/api/dashboards/:dashboardId/widgets", requireAuth, async (req, res) => {
    try {
      ...
      const [newWidget] = await db.insert(widgets)
        .values(widgetData)
        .returning();
  ...
  app.put("/api/widgets/:id", requireAuth, async (req, res) => {
    try {
      ...
      const [dashboard] = await db.select()
        .from(dashboards)
        .where(eq(dashboards.id, widget.dashboardId));
      ...
      const [updatedWidget] = await db.update(widgets)
        .set(updateData)
        .where(eq(widgets.id, id))
        .returning();
  ...
  app.post("/api/external-systems/test-config", requireAuth, async (req, res) => {
    try {
      ...
      res.status(500).json({ message: "Failed to test external system configuration" });
    }
  });
  ...
  app.get("/api/license-pools", requireAuth, async (req, res) => {
    try {
      const licensePools = await storage.getAllLicensePools();
      res.json(licensePools);
    } catch (error) {
...
// TODO: Duplicate route removed - app.put("/api/license-pools/:id", requireManagerOrAbove, async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const poolData = req.body;
      const updatedPool = await storage.updateLicensePool(id, poolData);
...
// TODO: Duplicate route removed - app.get("/api/certificates-of-compliance", requireAuth, async (req, res) => {
    try {
      ...
      if (contractId) {
        whereConditions.push(eq(certificatesOfCompliance.contractId, parseInt(contractId as string)));
      }
      
      if (serviceScopeId) {
        whereConditions.push(eq(certificatesOfCompliance.serviceScopeId, parseInt(serviceScopeId as string)));
      }
      
      if (status) {
        whereConditions.push(eq(certificatesOfCompliance.status, status as string));
      }
      ...
      console.error("Get certificates of compliance error:", error);
      res.status(500).json({ message: "Failed to fetch certificates of compliance" });
    }
  });
...
// TODO: Duplicate route removed - app.post("/api/certificates-of-compliance", requireManagerOrAbove, async (req, res) => {
    try {
      ...
      const [createdCOC] = await db
        .insert(certificatesOfCompliance)
        .values({
          clientId,
          contractId: contractId || null,
          serviceScopeId: serviceScopeId || null,
          safId: safId || null,
          cocNumber,
          title,
          description: description || '',
      ...
      res.status(201).json(createdCOC);
    } catch (error) {
      console.error("Create certificate of compliance error:", error);
      res.status(500).json({ message: "Failed to create certificate of compliance" });
    }
  });
...
// TODO: Duplicate route removed - app.delete("/api/certificates-of-compliance/:id", requireManagerOrAbove, async (req, res) => {
    try {
      ...
      console.error("Delete certificate of compliance error:", error);
      res.status(500).json({ message: "Failed to delete certificate of compliance" });
    }
  });
