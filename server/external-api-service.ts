...
// Create an HTTPS agent that ignores SSL certificate errors for internal systems
...
export class ExternalApiService {
  ...
  private getFetchOptions(headers: Record<string, string>, timeout: number = 10000) {
    return {
      ...
      timeout,
      agent: (url: string) => {
        // Use custom HTTPS agent for HTTPS requests to bypass SSL certificate issues
