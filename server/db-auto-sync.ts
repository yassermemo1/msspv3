  ...
  private async applyMigration(filename: string): Promise<void> {
    ...
    try {
      ...
      try {
        ...
        await client.query(
          'INSERT INTO schema_versions (script_version, schema_version, environment, migration_file, notes) VALUES ($1, $2, $3, $4, $5)',
          ['auto-sync', '1.4.3', this.options.environment, filename, `Auto-applied migration: ${filename}`]
        );
    ...
    try {
      ...
      await this.pool.query(
        `INSERT INTO schema_versions (script_version, app_version, schema_version, environment, notes) 
         VALUES ($1, $2, $3, $4, $5)`,
        ['auto-sync', appVersion, appVersion, this.options.environment, 'Auto-sync schema update completed']
      );
